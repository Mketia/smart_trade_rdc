...
export const Incentives: React.FC<IncentivesProps> = () => {
  ...
  // Quiz State
  const [quizIncentiveId, setQuizIncentiveId] = useState<string>('eac_str');
  const [checkedRequirements, setCheckedRequirements] = useState<Record<string, boolean>>({});

  // Filter incentives
  const filteredIncentives = TAX_INCENTIVES.filter((inc) => {
    const matchesSearch = 
      inc.titleKey.toLowerCase().includes(searchQuery.toLowerCase()) ||
      inc.descriptionKey.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const activeQuizIncentive = TAX_INCENTIVES.find(inc => inc.id === quizIncentiveId) || TAX_INCENTIVES[0];
  ...
  const handleToggleRequirement = (reqKey: string) => {
    ...
    const allReqs = activeQuizIncentive.requirementsKeys;
  ...
  const currentQuizRequirements = activeQuizIncentive.requirementsKeys;
  const tickedCount = currentQuizRequirements.filter((r: string) => checkedRequirements[r]).length;
  const isFullyQualified = tickedCount === currentQuizRequirements.length;
            ...
            filteredIncentives.map((inc) => (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-dashed border-zinc-150 dark:border-zinc-800">
                  ...
                  <button
                    onClick={() => handleIncentiveChangeForQuiz(inc.id)}
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800/80 rounded-2xl p-5 md:p-6 text-center space-y-4">
              ...
              <div className="w-full bg-zinc-200 dark:bg-zinc-800 rounded-full h-1.5">
                <div 
                  ...
                  style={{ width: `${(tickedCount / currentQuizRequirements.length) * 100}%` }}
                />
              </div>
